...
class FormsNewSubmit extends React.Component {
    ...
    return (
      <div>
        <Typography className={classes.copy} paragraph>
          <strong>Error [{saveFormError.status}]</strong>
          {saveFormError.message}
        </Typography>

        <div className={classes.buttonsWrapper}>
          <Button onClick={this.onGoBack}>Go Back</Button>
          <Button color="secondary" onClick={this.onSubmit}>
            {systemLocaleMap[locale].try_again}
          </Button>
        </div>
      </div>
    );
  ...
  renderSuccess = () => {
    ...
    return (
      <div>
        <Typography className={classes.copy} paragraph>
          {systemLocaleMap[locale].form_submitted}
        </Typography>
        ...
        <div className={classes.buttonsWrapper}>
          ...
          <Link href="/forms">
            <Button color="secondary">{systemLocaleMap[locale].finish}</Button>
          </Link>
        </div>
      </div>
    );
    ...
    return (
      <Paper className={classes.wrapper} square>
        <Typography variant="h6" color="primary">
          {systemLocaleMap[locale].submit}
        </Typography>
        <Typography className={classes.hint}>
          {systemLocaleMap[locale].placeholder}
        </Typography>
...
export default connect(mapStateToProps)(
  withStyles(styles, { withTheme: true })(FormsNewSubmit)
);
