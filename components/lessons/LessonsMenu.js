...
class LessonsMenu extends React.Component {
  state = {
    categorySelected:
      !!this.props.router.query && !!this.props.router.query.category
  ...
  handleCategorySelect = (category) => (e) => {
    ...
    if (keys.length === 1 && keys[0] === "content") {
      const file = content[locale][category].content.find(
        (file) => file.filename.indexOf(".md") > -1
      );
  ...
  renderMenuCategory = (category, i) => {
    ...
    const subcategories = Object.keys(content[locale][category]).filter(
      ...
      <div key={i} className={isSelected ? classes.menuListItemSelected : ""}>
        <ListItem button onClick={this.handleCategorySelect(category)}>
          ...
          <ListItemText
            className={classes.menuListItemText}
            inset
            primary={contentLocaleMap[locale][category]}
          />
  ...
  render() {
    ...
    else if (getContentError || getContentLocaleMapError)
      return (
        <ErrorMessage error={getContentError || getContentLocaleMapError} />
      );

    return (
      <div
        className={classNames(classes.menuWrapper, {
          [classes.menuWrapperOpened]: lessonsMenuOpened,
        })}
      >
        <div
          className={classes.menuToggleMobile}
        ...
        <List component="nav" className={classes.menuList}>
            ...
            <Link href="/lessons/favorites">
              <ListItem button>
                ...
                <ListItemText
                  className={classes.menuListItemText}
                  inset
                  primary={systemLocaleMap[locale].favorites_title}
                />
              </ListItem>
            </Link>
          </div>

          {Object.keys(content[locale])
            .filter(
              (category) =>
                !["content", "glossary", "pathways", "forms"].includes(category)
            )
          ...
          <div
            className={
              categorySelected == "glossary" ? classes.menuListItemSelected : ""
            }
          >
            <Link href={`/lessons/${locale}/glossary`}>
              <ListItem button>
                ...
                <ListItemText
                  className={classes.menuListItemText}
                  inset
                  primary={contentLocaleMap[locale]["glossary"]}
                />
              </ListItem>
            </Link>
          </div>
...
export default withRouter(
  connect(mapStateToProps)(withStyles(styles, { withTheme: true })(LessonsMenu))
);
