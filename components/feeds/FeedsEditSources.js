...
class FeedsEditSources extends React.Component {
  ...
  handleSelect = (sourceValue) => () => {
    ...
    if (sourcesSelected.includes(sourceValue)) {
      this.setState({
        sourcesSelected: sourcesSelected.filter(
          (value) => value !== sourceValue
        ),
      });
    } else
      this.setState({ sourcesSelected: sourcesSelected.concat([sourceValue]) });
  };

  handleSelectAll = (sources) => {
    const { sourcesSelected } = this.state;

    if (sourcesSelected.length === sources.length)
      ...
      <Paper className={classes.container} square>
        <form onSubmit={this.handleSubmit}>
          <Typography variant="h6">
            {systemLocaleMap[locale].feed_source_name}
          </Typography>
          <FormControl
            required
            error={error}
          ...
          <FormControl className={classes.buttonsWrapper} fullWidth>
            <Button component="button" onClick={this.handleCancel}>
              {systemLocaleMap[locale].cancel}
            </Button>
            <ClickAwayListener onClickAway={this.handleRemoveError}>
              <Button color="secondary" onClick={this.handleSubmit}>
                {systemLocaleMap[locale].ok}
              </Button>
            </ClickAwayListener>
          </FormControl>
        </form>
...
export default connect(mapStateToProps)(
  withStyles(styles, { withTheme: true })(FeedsEditSources)
);
