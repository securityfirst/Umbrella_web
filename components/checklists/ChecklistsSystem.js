...
import {
  toggleChecklistFavorite,
  deleteChecklistSystem,
...
class ChecklistsSystem extends React.Component {
    ...
    const check = async (item) => {
      ...
      if (item.filename === "c_checklist.yml") {
        const res = await fetch(
          `${process.env.ROOT}/api/github/content/${item.sha}`
        );
        ...
        if (checklist) {
          ...
          this.setState({
            checklistCount: this.state.checklistCount + checklist.list.length,
          });
        }
  ...
  renderPanelLink = (title, percentage, index) => {
    ...
    const checklistFile = content[locale][cat][subcat][level].content.find(
      (f) => f.filename === "c_checklist.yml"
    );
    ...
    return (
      <div className={classes.panelWrapper} key={index}>
        <Link href={`/lessons/${locale}/${title.replace(/ > /, "/")}`}>
          ...
          >
            <img
              ...
              src={`/static/assets/images/${level}.png`}
              alt={`Umbrella lesson ${level} icon`}
            />
            <Typography className={classes.panelTitle} variant="h6">
              {title.replace(/\./g, " > ").replace(/-/g, " ")}
            </Typography>
            {percentage !== null && (
              <Typography className={classes.panelPercentage} variant="h6">
                {percentage}%
              </Typography>
            )}
          </Button>
        ...
        <IconButton
          ...
          aria-label="Delete"
          onClick={this.deleteChecklist(title)}
        >
          <DeleteIcon />
        </IconButton>
      </div>
  ...
  renderLessonChecklistFavorites = () => {
    ...
    if (!checklistsSystemKeys.length)
      return this.renderPanel(
        systemLocaleMap[locale].checklist_favorites_empty
      );
        ...
        {checklistsSystemKeys.map((name, i) => {
          ...
          if (checklists.length) {
            checklist = checklists.find((set) => {
              return set.list.find(
                (item) => item.check === checklistsSystem[name].items[0]
              );
            });

            if (checklist)
              checklistCount = checklist.list.reduce(
                (acc, item) => (!!item.check ? acc + 1 : acc),
                0
              );
          }

          const percentage = !!checklist
            ? parseInt(
                (checklistsSystem[name].items.length / checklistCount) * 100
              )
            : 0;
  ...
  renderLessonChecklists = () => {
    ...
    if (!checklistsSystemKeys.length)
      return this.renderPanel(systemLocaleMap[locale].checklists_unavailable);
    ...
    return (
      <React.Fragment>
        {!pathwaysSaved.length ? (
          <Paper className={classes.panel}>
            <Typography className={classes.pathwayTitle} variant="h6">
              {systemLocaleMap[locale].checklist_pathways_empty}
            </Typography>
          </Paper>
        ) : (
          pathwaysSaved
        ...
        <Typography
          className={classes.pathwaySeeAll}
          onClick={this.openPathwaysModal}
        >
          {systemLocaleMap[locale].checklist_see_all}
        </Typography>
      </React.Fragment>
    );
  };
  ...
  render() {
    ...
    else if (getChecklistsSystemError)
      return <ErrorMessage error={getChecklistsSystemError} />;

    const totalDone = Object.keys(checklistsSystem).reduce(
      (acc, key) => acc + checklistsSystem[key].items.length,
      0
    );

    let totalDonePercentage = (totalDone / checklistCount) * 100;

    if (totalDonePercentage !== 0 && totalDonePercentage < 1)
      totalDonePercentage = "< 1";
    else totalDonePercentage = parseInt(totalDonePercentage);

    return (
      <div className={classes.content}>
        <Typography className={classes.label} variant="subtitle1">
          {systemLocaleMap[locale].checklist_total_title}
        </Typography>

        <Paper className={classes.panel}>
          <div className={classes.panelTotalIconsWrapper}>
            <img
              className={classes.panelTotalIcon}
              src={`/static/assets/images/beginner.png`}
              alt={`Umbrella lesson beginner icon`}
            />
            <img
              className={classNames(
                classes.panelTotalIcon,
                classes.panelTotalIconSecond
              )}
              src={`/static/assets/images/advanced.png`}
              alt={`Umbrella lesson intermediate icon`}
            />
            <img
              className={classNames(
                classes.panelTotalIcon,
                classes.panelTotalIconThird
              )}
              src={`/static/assets/images/expert.png`}
              alt={`Umbrella lesson expert icon`}
            />
          </div>
          <Typography className={classes.panelTitle} variant="h6">
            {systemLocaleMap[locale].checklist_total_done}
          </Typography>
          <Typography className={classes.panelPercentage} variant="h6">
            {totalDonePercentage}%
          </Typography>
        ...
        <Typography className={classes.label} variant="subtitle1">
          {systemLocaleMap[locale].favorites_title}
        </Typography>

        {this.renderLessonChecklistFavorites()}

        <Typography className={classes.label} variant="subtitle1">
          {systemLocaleMap[locale].checklist_lessons_title}
        </Typography>

        {this.renderLessonChecklists()}

        {!!content[locale].pathways && (
          <Typography className={classes.label} variant="subtitle1">
            {systemLocaleMap[locale].checklist_pathways_title}
          </Typography>
        )}
...
export default connect(mapStateToProps)(
  withStyles(styles, { withTheme: true })(ChecklistsSystem)
);
