...
class ChecklistsPanel extends React.Component {
  ...
  addItem = (e) => {
    ...
    if (!itemText || !itemText.length) {
      return dispatch(
        openAlert(
          "error",
          systemLocaleMap[locale].checklist_custom_empty_item_error_message
        )
      );
    }
    ...
    const newChecklist = {
      ...checklist,
      items: checklist.items.concat([item]),
    };
  ...
  downloadHtml = (checklist, percentage) => async () => {
    ...
    try {
      await dispatch(
        openAlert("info", systemLocaleMap[locale].form_downloading)
      );
  ...
  render() {
    ...
    if (!checklist.items || !checklist.items.length) percentage = 0;
      ...
      const itemsDone = checklist.items.reduce(
    ...
    return (
      <ExpansionPanel
        ...
        onChange={handlePanelToggle(index)}
      >
        ...
        >
          ...
          <div className={classes.rightWrapper}>
            ...
            >
              {systemLocaleMap[locale].download_title}
            </Button>
            ...
            >
              <ListItem
                button
                onClick={this.downloadHtml(checklist, percentage)}
              >
                <ListItemText primary={systemLocaleMap[locale].html_name} />
              </ListItem>
              <ListItem
                button
                onClick={this.downloadPdf(checklist, percentage)}
              >
                <ListItemText primary={systemLocaleMap[locale].pdf_name} />
              </ListItem>
              <ListItem
                button
                onClick={this.downloadDocx(checklist, percentage)}
              >
          ...
          <form className={classes.form} onSubmit={this.addItem}>
            {checklist.items && checklist.items.length ? (
              <FormControl className={classes.formControl} component="fieldset">
                <FormGroup>{checklist.items.map(this.renderItem)}</FormGroup>
              </FormControl>
            ) : (
              <Typography className={classes.empty} paragraph>
                {systemLocaleMap[locale].checklist_empty}
              </Typography>
            )}
            ...
            {isCustom && (
              <div className={classes.addItemWrapper}>
                ...
                <Button component="button" size="small" onClick={this.addItem}>
                  {systemLocaleMap[locale].checklist_add_item_title}
                </Button>
              </div>
            )}
...
export default connect(mapStateToProps)(
  withStyles(styles, { withTheme: true })(ChecklistsPanel)
);
