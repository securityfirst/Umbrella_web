...
export const getLessonCardsFavorites = () => async (dispatch, getState) => {
  ...
  try {
    ...
    await ClientDB.default
      ...
      .then(async (lessons) => {
        await dispatch(
          fulfilled(lessonsTypes.GET_LESSON_CARDS_FAVORITES, lessons || [])
        );
      })
      .catch(async (err) => {
        dispatch(openAlert("error", systemLocaleMap[locale].general_error));
...
export const addLessonCardFavorite = (file, category, level) => (
  dispatch,
  getState
) => {
  ...
  if (
    state.lessons.lessonCardsFavorites.find((item) => item.name === file.name)
  ) {
  ...
  const favorites = state.lessons.lessonCardsFavorites.concat([
    { ...file, category, level },
  ]);
  ...
  try {
    ...
    ClientDB.default
      ...
      .then(() => {
        dispatch(
          openAlert("success", systemLocaleMap[locale].lesson_favorite_added)
        );
        return dispatch(
          fulfilled(lessonsTypes.ADD_LESSON_CARD_FAVORITE, favorites)
        );
      })
      .catch((err) => {
        dispatch(openAlert("error", systemLocaleMap[locale].general_error));
...
export const removeLessonCardFavorite = (file) => (dispatch, getState) => {
  ...
  if (state.account.isProtected && !state.account.password) {
    ...
    dispatch(openAlert("error", message));
    return dispatch(
      rejected(lessonsTypes.REMOVE_LESSON_CARD_FAVORITE, message)
    );
  }
  ...
  const favorites = state.lessons.lessonCardsFavorites.filter(
  ...
  try {
    ...
    ClientDB.default
      ...
      .then(() => {
        dispatch(
          openAlert("success", systemLocaleMap[locale].lesson_favorite_removed)
        );
        return dispatch(
          fulfilled(lessonsTypes.REMOVE_LESSON_CARD_FAVORITE, favorites)
        );
      })
      .catch((err) => {
        dispatch(openAlert("error", systemLocaleMap[locale].general_error));
...
export const unsetLessonChecklist = () => ({
  type: lessonsTypes.UNSET_LESSON_CHECKLIST,
});
