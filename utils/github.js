...
export const formatContentUrls = ({
  blob = "",
  locale = "en",
  category = "",
  level = "",
  ...
  let formattedContent = replacedContent.replace(urlRegex, (url) => {
    ...
    if (path.indexOf(".md") == path.length - 3) {
      ...
      if (path.indexOf("tools") === 0) {
        const lesson = content[locale]["tools"][subcategory].content.find(
  ...
  const imageUrlRegex = /\(([^\)]+)\)/gi;

  formattedContent = formattedContent.replace(imageUrlRegex, (url) => {
    let strings = url.split(".");

    if (
      ["jpg)", "jpeg)", "png)", "svg)", "gif)", "bmp)"].includes(strings[1])
    ) {
      let subpath = "";

      if (category == "-") subpath = "about";
      else if (category.indexOf("tools") > -1)
        subpath = `${category.replace(".", "/")}`;
      else subpath = `${category.replace(".", "/")}/${level}`;

      return `(https://raw.githubusercontent.com/securityfirst/umbrella-content/master/en/${subpath}/${url
        .replace("(", "")
        .replace(")", "")})`;
    }
...
export const getNameFromFilenameYml = (filename) => {
  return !!filename
    ? filename.slice(2).replace(/\.yml/, "").replace(/-/g, " ")
    : "";
};
