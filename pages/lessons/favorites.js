...
class LessonsFavorites extends React.Component {
  ...
  render() {
    ...
    else if (getLessonCardsFavoritesError)
      return <ErrorMessage error={getLessonCardsFavoritesError} />;
        ...
        <div className={classes.wrapper}>
          ...
          >
            <Typography className={classes.label} variant="subtitle1">
              {systemLocaleMap[locale].lesson_favorites_title}
            </Typography>

            {!lessonCardsFavorites.length ? (
              <Paper className={classes.paper}>
                <Typography>
                  {systemLocaleMap[locale].lesson_empty_favorites_message}
                </Typography>
              </Paper>
            ) : (
...
export default connect(mapStateToProps)(
  withStyles(styles, { withTheme: true })(LessonsFavorites)
);
