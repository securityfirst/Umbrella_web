...
class LessonCard extends React.Component {
  ...
  renderNavigation = () => {
    ...
    const lessons =
      cat === "glossary"
        ? content[locale].glossary.content.filter(
            (l) => l.filename.indexOf("s_") === 0
          )
        : level !== "-"
        ? content[locale][cat][subcat][level].content.filter(
            (l) => l.filename.indexOf("s_") === 0
          )
        : content[locale][cat][subcat].content.filter(
            (l) => l.filename.indexOf("s_") === 0
          );
        ...
        <Toolbar>
            ...
            >
              {lessons.map((lesson, i) => {
                return (
                  <Link
                    key={i}
                    href={`/lessons/${locale}/${category}/${level}/${lesson.sha}`}
                  >
                    <MenuItem
          ...
          {isLast ? (
            ...
            <div className={classes.grow}>
              {!isFirst && (
                <Link
                  href={`/lessons/${locale}/${category}/${level}/${
                    (lessons[lessonIndex - 1] || {}).sha
                  }`}
                >
                  <Button color="inherit">
  ...
  renderContent = () => {
    const {
      ...
      content,
      currentLessonFile,
      getLessonFileLoading,
      getLessonFileError,
    } = this.props;
    const { locale, category, level } = router.query;

    if (getLessonFileLoading) return <Loading />;
    else if (getLessonFileError)
      return <ErrorMessage error={getLessonFileError} />;

    const formattedContent = formatContentUrls({
      blob: currentLessonFile,
      locale,
      category,
      level,
      content,
...
export default withRouter(
  connect(mapStateToProps)(withStyles(styles, { withTheme: true })(LessonCard))
);
