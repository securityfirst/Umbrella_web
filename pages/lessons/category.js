...
class LessonsCategory extends React.Component {
  ...
  render() {
    ...
    for (let i = 0; i < breadcrumb.length; i++) {
      breadcrumb[i] = contentLocaleMap[locale][breadcrumb[i]].replace(
        /-/g,
        " "
      );
    }
    ...
    return (
      ...
      >
        <div className={classes.wrapper}>
          ...
          >
            <Typography className={classes.breadcrumb} variant="subtitle1">
              {breadcrumb}
            </Typography>
...
export default withRouter(
  connect(mapStateToProps)(
    withStyles(styles, { withTheme: true })(LessonsCategory)
  )
);
