...
import {
  getLessonChecklist,
  unsetLessonChecklist,
} from "../../store/actions/lessons";
import {
  getChecklistsSystem,
  updateChecklistsSystem,
...
class LessonsLevel extends React.Component {
  ...
  componentDidMount() {
    ...
    const lesson = get(
      content,
      level !== "-" ? `${locale}.${category}.${level}` : `${locale}.${category}`
    );
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.locale !== this.props.locale) {
      this.props.dispatch(
        setAppbarTitle(
          this.props.systemLocaleMap[nextProps.locale].lesson_title
        )
      );
    }
  }
  ...
  renderChecklist = () => {
    ...
    else if (getLessonChecklistError)
      return (
        <ErrorMessage
          error={getLessonChecklistError || getChecklistsSystemError}
        />
      );
    ...
    return (
      <Card className={classes.checklistCard}>
        <CardContent className={classes.checklistCardHead}>
          <Typography className={classes.checklistCardTitle}>
            {systemLocaleMap[locale].checklist_detail_title}
          </Typography>
...
export default withRouter(
  connect(mapStateToProps)(
    withStyles(styles, { withTheme: true })(LessonsLevel)
  )
);
