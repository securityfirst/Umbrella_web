        ...
        <div className={classes.formPanelButtonsWrapper}>
          <div>
            <Typography className={classes.label}>
              <strong>{systemLocaleMap[locale].date_created}</strong>:{" "}
              {new Date(formSaved.dateCreated).toLocaleString()}
            </Typography>
            {formSaved.dateModified && (
              <Typography className={classes.label}>
                <strong>{systemLocaleMap[locale].date_modified}</strong>:{" "}
                {new Date(formSaved.dateModified).toLocaleString()}
              </Typography>
            )}
          </div>
          <div>
            <Button
              component="button"
              aria-owns={
                newAnchorEl ? `active-download-menu-${randomNumber}` : undefined
              }
              aria-haspopup="true"
            ...
            <Link href={`/forms/${formSaved.sha}/${formSaved.id}`}>
              <Button component="button">
                {systemLocaleMap[locale].form_menu_edit}
              </Button>
            </Link>
            <Button
              component="button"
              color="primary"
              onClick={this.delete(formSaved)}
            >
...
class AvailablePanel extends React.Component {
  ...
  render() {
    const { form, classes, locale, systemLocaleMap, checkLogin } = this.props;
        ...
        <div className={classes.formPanelButtonsWrapper}>
          ...
          <div>
            ...
            <Link href={`/forms/${form.sha}`}>
              <Button color="primary" onClick={checkLogin}>
                {systemLocaleMap[locale].form_message_body_form_new}
              </Button>
            </Link>
            <Menu
...
class Forms extends React.Component {
  state = {
    forms: this.props.content[this.props.locale].forms.content
      ...
      .map((form) => ({
        sha: form.sha,
        title: form.filename
          .slice(2, form.filename.length - 4)
          .replace(/-/g, " "),
      })),
  };
  ...
  setFormData = async ({ locale }) => {
    this.props.dispatch(
      setAppbarTitle(this.props.systemLocaleMap[locale].form_title)
    );

    const forms = this.props.content[locale].forms.content.filter(
      (form) => form.filename.indexOf("f_") === 0
    );

    let formsList = [];

    for (let i = 0; i < forms.length; i++) {
      let formData = await fetch(
        `${process.env.ROOT}/api/github/content/${forms[i].sha}`
      );
  ...
  render() {
    ...
    else if (getContentError || getFormsSavedError)
      return <ErrorMessage error={getContentError || getFormsSavedError} />;
    ...
    return (
      ...
      >
        <div className={classes.content}>
          {!!formsSaved.length && (
            <React.Fragment>
              <Typography className={classes.label} variant="subtitle1">
                {systemLocaleMap[locale].form_title}
              </Typography>
          ...
          <Typography className={classes.label} variant="subtitle1">
            {systemLocaleMap[locale].form_message_title_all_forms}
          </Typography>
...
export default connect(mapStateToProps)(
  withStyles(styles, { withTheme: true })(Forms)
);
