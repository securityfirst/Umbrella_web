...
class Account extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.locale !== this.props.locale) {
      this.props.dispatch(
        setAppbarTitle(
          nextProps.systemLocaleMap[nextProps.locale].account_title
        )
      );
    }
  }
  ...
  handleFormOpen = (type) => () => {
    ...
    switch (type) {
      case "location":
        state.feedsModalContent = (
          <FeedsEditLocation closeModal={this.handleModalClose} />
        );
        break;
      case "sources":
        state.feedsModalContent = (
          <FeedsEditSources closeModal={this.handleModalClose} />
        );
        break;
    }
  ...
  checkPassword = (type) => {
    ...
    if (value.length < 8 || !RegExp("[A-Z]+[0-9]*").test(value)) {
      this.setState({
        [`${type}Error`]: true,
        [`${type}ErrorMessage`]: systemLocaleMap[locale]
          .account_password_alert_description,
      });
    ...
    if (
      ...
      if (passwordExists) {
        if (this.checkPassword("passwordOld")) {
          dispatch(
            resetPassword(password, passwordOld, () => {
              this.setState({
                password: "",
                passwordConfirm: "",
                passwordOld: "",
  ...
  unsetPassword = (e) => {
    ...
    if (!passwordExists || !isProtected) {
      return dispatch(
        openAlert("warning", systemLocaleMap[locale].account_password_not_set)
      );
    }
      ...
      <div>
        <div className={classes.settingsRow}>
          <div className={classes.settingsColumnLeft}>
            ...
            >
              {systemLocaleMap[locale].account_select_language}
            </Button>
        ...
        <div className={classes.settingsRow}>
          <div className={classes.settingsColumnLeft}>
            <Button color="primary" onClick={this.clearDb}>
              {systemLocaleMap[locale].account_delete_cache}
            </Button>
          </div>
          <div className={classes.settingsColumnRight}>
  ...
  renderFeedSettings = () => {
    ...
    return (
      <div style={{ width: "100%" }}>
        <div className={classes.settingsRow}>
          <div className={classes.settingsColumnLeft}>
            <Button color="primary" onClick={this.handleFormOpen("location")}>
              {systemLocaleMap[locale].feed_location_label}
            </Button>
          </div>
          <div className={classes.settingsColumnRight}>
            <Typography>
              {feedLocation ? feedLocation.place_name : "-"}
            </Typography>
          </div>
        </div>
        ...
        <div className={classes.settingsRow}>
          <div className={classes.settingsColumnLeft}>
            <Button color="primary" onClick={this.handleFormOpen("sources")}>
              {systemLocaleMap[locale].feed_source_label}
            </Button>
          </div>
          <div className={classes.settingsColumnRight}>
            <Typography>
              {feedSources.length
                ? `${feedSources.length} source${
                    feedSources.length > 1 ? "s" : ""
                  }`
                : "-"}
            </Typography>
          </div>
        ...
        <div className={classes.buttonWrapper}>
          ...
          {passwordExists && isProtected && (
            <Button
              ...
              color="primary"
              onClick={this.unsetPassword}
            >
              {systemLocaleMap[locale].account_title_skip_pw}
            </Button>
          )}
        </div>
      </form>
    ...
    return (
      <React.Fragment>
        <Typography paragraph>
          <strong>{systemLocaleMap[locale].account_status}: </strong>
          {passwordExists
            ? systemLocaleMap[locale].account_password_success
            : systemLocaleMap[locale].account_title_skip_pw}
        </Typography>
        <Typography className={classes.disclaimerLarge}>
          <strong>{systemLocaleMap[locale].account_disclaimer}: </strong>{" "}
          {systemLocaleMap[locale].account_disclaimer_description}
        </Typography>
  ...
  render() {
    ...
    return (
      <Layout
        title={`${systemLocaleMap[locale].app_name} | ${systemLocaleMap[locale].account_title}`}
      ...
      >
        <div className={classes.content}>
          ...
          <ExpansionPanel
            ...
            onChange={this.handlePanelToggle(0)}
          >
            <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
              <Typography className={classes.heading} variant="h6">
                {systemLocaleMap[locale].account_settings}
              </Typography>
          ...
          <ExpansionPanel
            ...
            onChange={this.handlePanelToggle(1)}
          >
            <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
              <Typography className={classes.heading} variant="h6">
                {systemLocaleMap[locale].feed_set_your_feed}
              </Typography>
            </ExpansionPanelSummary>
            <ExpansionPanelDetails>
          ...
          <ExpansionPanel
            ...
            onChange={this.handlePanelToggle(2)}
          >
            <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
              <Typography className={classes.heading} variant="h6">
                {passwordExists
                  ? systemLocaleMap[locale].account_reset_password
...
export default withRouter(
  connect(mapStateToProps)(withStyles(styles, { withTheme: true })(Account))
);
